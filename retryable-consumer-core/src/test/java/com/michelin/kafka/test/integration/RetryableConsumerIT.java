...
public class RetryableConsumerIT {
    ...
    public static void setupKafkaConfig() {
        ...
        Properties retryableConsumerKafkaProps = kafkaRetryableConfiguration.getConsumer().getProperties();
        ...
        retryableConsumerKafkaProps.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class.getName());
        retryableConsumerKafkaProps.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class.getName());
    ...
    void happyPath() throws ExecutionException, InterruptedException {
        ...
        //WHEN
        try(RetryableConsumer<String, String> retryableConsumer = new RetryableConsumer<>(kafkaRetryableConfiguration)) {
            ...
            assertTimeout(
                    ...
                    "Expected number of records ("+numberOfRecordToProduce+") " +
    ...
    void notRetryableError() throws ExecutionException, InterruptedException {
        ...
        kafkaRetryableConfiguration.getConsumer().setNotRetryableExceptions(Collections.singletonList(NotRetryableCustomException.class.getName()));
        try (RetryableConsumer<String, String> retryableConsumer = new RetryableConsumer<>(kafkaRetryableConfiguration)) {
            ...
            try (KafkaConsumer<String, GenericErrorModel> dlqConsumer =
                ...
                Assertions.assertTimeoutPreemptively(
                    Duration.ofSeconds(15), //Wait for 15 seconds max to have the expected number of record in DL topic
                    () -> {
                        while(true) {
     ...
     */
    private List<ConsumerRecord<String, GenericErrorModel>> getDeadLetterContent(String deadLetterTopic, Duration consumerTimeout) {
        ...
        try (KafkaConsumer<String, GenericErrorModel> dlqConsumer =
            ...
            while (true) {
                ...
                if (Duration.ofMillis(elapsedTime).compareTo(consumerTimeout) > 0)
                    return recordsList;
            }
        }
    }
    ...
    static class NotRetryableCustomException extends Exception {
        ...
        protected NotRetryableCustomException(String message, Throwable cause, boolean enableSuppression, boolean writableStackTrace) {
