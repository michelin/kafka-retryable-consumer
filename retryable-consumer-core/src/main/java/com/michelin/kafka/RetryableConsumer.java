...
public class RetryableConsumer<K, V> implements Closeable {
    ...
    /**
     * Map saving the current offset Of TopicPartitions treated by this consumer.
     * This offsets map is used to manage rewind in case of error or retry
     */
    ...
    public RetryableConsumer(KafkaRetryableConfiguration kafkaRetryableConfiguration) {
        ...
        this.consumer = new KafkaConsumer<>(this.kafkaRetryableConfiguration.getConsumer().getProperties());
        this.errorHandler = new RetryableConsumerErrorHandler<>(new DeadLetterProducer(this.kafkaRetryableConfiguration.getDeadLetter()),
     ...
     */
    protected void skipCurrentOffset(
     ...
     * Offsets are saved according to the name of its belonging topic
     * and the number of its belonging partition.
     *
     ...
     * Handle closing down exceptions and CommitFailedExceptions in case of rebalance
     * <p>
     * <a href="https://docs.confluent.io/current/clients/java.html#synchronous-commits">Synchronous Commits on confluent docs</a>
     */
    protected void doCommitSync() {
        try {
            ...
            log.warn("Commit failed Normal : due to rebalance. If this persists there may be issues with configuration or infrastructure", e);
        }
    }
            ...
            if(offsets.containsKey(tp)) { //some offset has been processed for this partition
                if(offsets.get(tp)!=null) {
                    ...
                    log.warn(
                            ...
                            "this could happen if the consumer group was just created", tp.toString()
                    );
                }
            } else {
    ...
    public void listen(RecordProcessor<ConsumerRecord<K, V>, Exception> recordProcessor) {
        ...
        this.listen(
                kafkaRetryableConfiguration.getConsumer().getTopics(),
                recordProcessor
        );
    }
    ...
    private void processRecords(RecordProcessor<ConsumerRecord<K, V>, Exception> recordProcessor, ConsumerRecords<K, V> records) throws Exception {
